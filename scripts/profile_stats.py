...
def start(w,h,t):return f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 {w} {h}"><rect width="100%" height="100%" rx="14" fill="#101513"/><text x="28" y="34" fill="#d1fae5" font-family="Arial" font-size="16" font-weight="700">{esc(t)}</text>'
...
for i,(lab,val) in enumerate(metrics):
 x=28+i*174;s+=f'<text x="{x}" y="78" fill="#9ca3af" font-family="Arial" font-size="11">{lab}</text><text x="{x}" y="112" fill="#6ee7b7" font-family="Arial" font-size="27" font-weight="700">{val:,}</text>'
...
for lang,val in langs.items():
 pct=val/total;width=int(360*pct);s+=f'<text x="28" y="{y}" fill="#d1fae5" font-family="Arial" font-size="12">{esc(lang)}</text><rect x="150" y="{y-11}" width="360" height="14" rx="7" fill="#1c2924"/><rect x="150" y="{y-11}" width="{max(width,3)}" height="14" rx="7" fill="#34d399"/><text x="530" y="{y}" fill="#9ca3af" font-family="Arial" font-size="11">{pct*100:.1f}%</text>';y+=28
(OUT/'languages.svg').write_text(s+'</svg>',encoding='utf-8')
s=start(900,180,'CONTRIBUTION MATRIX');sx,sy,cell,gap=28,62,10,3;fills=['#1c2924','#204034','#287150','#34a875','#6ee7b7']
...
s+=f'<text x="28" y="158" fill="#9ca3af" font-family="Arial" font-size="11">{cal["totalContributions"]:,} contributions in the last year</text></svg>'; (OUT/'activity.svg').write_text(s,encoding='utf-8')
...
for i,(lab,val) in enumerate(ach):
 col,row=i%3,i//3;x=28+col*292;y=72+row*76;s+=f'<rect x="{x}" y="{y}" width="265" height="58" rx="9" fill="#1c2924" stroke="#2f4a3e"/><text x="{x+16}" y="{y+21}" fill="#9ca3af" font-family="Arial" font-size="10">{esc(lab)}</text><text x="{x+16}" y="{y+46}" fill="#6ee7b7" font-family="Arial" font-size="20" font-weight="700">{val:,}</text>'
